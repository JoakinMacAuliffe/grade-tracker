import { // I read somewhere that it's not efficient to import * so i'm stuck with this comically large import statement...
  pgTable, 
  pgEnum, 
  timestamp, 
  check, 
...
export const ramos = pgTable('ramos', {
  ...
  notaEximicion: decimal("notaEximicion", { precision: 2, scale: 1}).default(5.0),
  semestreId: integer("semestreId").references(() => semestres.id, { onDelete: "cascade" })
});
...
export const evaluaciones = pgTable('evaluaciones', {
  ...
  ramoId: integer("ramoId").notNull().references(() => ramos.id, { onDelete: "cascade" }),
  fecha: timestamp("fecha", { withTimezone: true })
}, (table) => ({
  // make sure the grades are within limits
  notaRange: check('nota_range', sql`${table.nota} >= 1.0 AND ${table.nota} <= 7.0`),
  // same for weights
  ponderacionRange: check('pond_range', sql`${table.ponderacion} >= 0.0 AND ${table.ponderacion} <= 1.0`)
}));
...
export const semestresRelations = relations(semestres, ({ many }) => ({ // 1 semester has N courses
  ramos: many(ramos)
}));

export const ramosRelations = relations(ramos, ({ one, many }) => ({ // 1 semester has N courses
...
export const seccionesRelations = relations(secciones, ({ one }) => ({ // 1 course has N sections
  ramo: one(ramos, {
...
export const evaluacionesRelations = relations(evaluaciones, ({ one }) => ({ // 1 course has N assignments
  ramo: one(ramos, {
